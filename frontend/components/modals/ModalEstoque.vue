<template>
  <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" @click.self="fechar">
    <div class="bg-white dark:bg-neutral-900/90 backdrop-blur-2xl border border-gray-200 dark:border-white/[0.08] w-full max-w-lg rounded-3xl overflow-hidden max-h-[90vh] flex flex-col shadow-float">

      <!-- HEADER -->
      <div class="p-6 border-b border-gray-100 dark:border-white/[0.08] flex justify-between items-center">
        <div>
          <h2 class="text-xl font-black text-gray-900 dark:text-white leading-tight">{{ produto?.nome }}</h2>
          <p class="text-sm text-gray-500 dark:text-white/50 mt-0.5">
            Estoque atual:
            <b class="text-gray-900 dark:text-white">{{ estoqueAtual }}</b>
            <span v-if="produto?.estoque_minimo > 0" class="text-gray-400 dark:text-white/30"> · mín. {{ produto.estoque_minimo }}</span>
          </p>
        </div>
        <button @click="fechar"
          class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-white/[0.06] hover:bg-red-950/40 text-gray-500 dark:text-white/60 font-bold transition-colors">
          ✕
        </button>
      </div>
      ...
      <div class="p-6 overflow-y-auto flex-1 space-y-5">
        ...
        <div class="grid grid-cols-3 gap-2">
          <button v-for="t in tipos" :key="t.value" @click="tipo = t.value"
            ...
            :class="tipo === t.value
              ? t.classeAtiva
              : 'border-gray-200 dark:border-white/10 text-gray-400 dark:text-white/40 hover:border-gray-300 dark:hover:border-white/25'">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label for="est-qtd" class="block text-xs font-bold text-gray-500 dark:text-white/40 mb-1">
            ...
            <input id="est-qtd" name="est-qtd" v-model.number="quantidade" type="number" min="0" step="1"
              ...
              class="w-full h-11 px-3 rounded-xl bg-gray-50 dark:bg-white/[0.06] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white outline-none focus:border-orange-500/70 transition-colors" />
          </div>
          <div>
            <label for="est-motivo" class="block text-xs font-bold text-gray-500 dark:text-white/40 mb-1">Motivo (opcional)</label>
            <input id="est-motivo" name="est-motivo" v-model="motivo" type="text" maxlength="150"
              ...
              class="w-full h-11 px-3 rounded-xl bg-gray-50 dark:bg-white/[0.06] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/25 outline-none focus:border-orange-500/70 transition-colors" />
          </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-3">
          <p class="text-sm text-gray-500 dark:text-white/50">
            <template v-if="novoEstoque !== null">
              Ficará com <b :class="novoEstoque <= (produto?.estoque_minimo || 0) ? 'text-orange-500' : 'text-gray-900 dark:text-white'">{{ novoEstoque }}</b> em estoque
            </template>
          </p>
        ...
        <!-- HISTÓRICO -->
        <div>
          <h3 class="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-white/30 mb-2">Últimas movimentações</h3>
          ...
          <p v-else-if="!movimentos.length" class="text-sm text-gray-400 dark:text-white/30 py-3 text-center">
            Nenhuma movimentação registrada ainda
          </p>

          <div v-else class="space-y-1.5">
            <div v-for="m in movimentos" :key="m.id"
              class="flex items-center gap-3 px-3 py-2 rounded-xl bg-gray-50 dark:bg-white/5">
              ...
              <span class="text-sm font-black shrink-0 w-12 text-right"
                :class="m.quantidade > 0 ? 'text-green-400' : 'text-red-400'">
                {{ m.quantidade > 0 ? '+' : '' }}{{ m.quantidade }}
              </span>
              <div class="flex-1 min-w-0">
                <p class="text-xs text-gray-500 dark:text-white/60 truncate">{{ m.motivo || '—' }}</p>
                <p class="text-[10px] text-gray-400 dark:text-white/30">
                  {{ formatarData(m.created_at) }}<template v-if="m.usuario"> · {{ m.usuario }}</template>
                </p>
              </div>
              <span v-if="m.estoque_resultante !== null" class="text-[10px] text-gray-400 dark:text-white/30 shrink-0">
...
const tipos = [
  { value: 'entrada', label: 'Entrada', icone: PackagePlus,
    classeAtiva: 'border-green-500 bg-green-950/40 text-green-400' },
  { value: 'saida', label: 'Saída', icone: PackageMinus,
    classeAtiva: 'border-red-500 bg-red-950/40 text-red-400' },
  { value: 'ajuste', label: 'Ajuste', icone: SlidersHorizontal,
    classeAtiva: 'border-blue-500 bg-blue-950/40 text-blue-400' }
]
...
function corTipo(t: string) {
  if (t === 'entrada')      return 'bg-green-950/50 text-green-400'
  if (t === 'saida')        return 'bg-red-950/50 text-red-400'
  if (t === 'ajuste')       return 'bg-blue-950/50 text-blue-400'
  if (t === 'venda')        return 'bg-orange-950/50 text-orange-400'
